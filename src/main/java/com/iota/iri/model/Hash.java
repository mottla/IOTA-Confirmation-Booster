...
public class Hash implements Serializable, Indexable {
    ...
    public static final Hash NULL_HASH = new Hash(new int[JCurl.HASH_LENGTH]);
    ...
    public synchronized void dooo(final int[] trits, final int offset) {
    ...
    public static Hash calculate(byte[] bytes, int tritsLength, final ICurl curl) {
        int[] trits = new int[tritsLength];
        Converter.getTrits(bytes, trits);
        return calculate(trits, 0, tritsLength, curl);
    }

    public static Hash calculate(final int[] tritsToCalculate, int offset, int length, final ICurl curl) {
    ...
    public int[] trits() {
        if (trits == null) {
            trits = new int[JCurl.HASH_LENGTH];
            Converter.getTrits(bytes, trits);
        }
        return trits;
    }
    ...
    private synchronized void fullRead(byte[] bytes, int offset, int size) {
