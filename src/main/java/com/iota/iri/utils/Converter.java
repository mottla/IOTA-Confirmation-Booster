...
public class Converter {
    ...
    public static void shiftPair(Pair<long[], long[]> source, Pair<long[], long[]> dest) {
        if(source.left.length == dest.left.length && source.right.length == dest.right.length) {
            for(int i = 0; i < dest.left.length; i++) {
                dest.left[i] <<= 1;
                dest.left[i] |= source.left[i] & 1;
            }
            for(int i = 0; i < dest.right.length; i++) {
                dest.right[i] <<= 1;
                dest.right[i] |= source.right[i] & 1;
            }
        }
    }
    ...
    public static int[] trits(final Pair<long[], long[]> pair, final int bitIndex) {
        final int length;
        if(pair.left.length == pair.right.length || pair.left.length < pair.right.length) {
        ...
        for(int i = 0; i < length; i++) {
            low = pair.left[i] & mask;
            hi = pair.right[i] & mask;
            if( hi == low ) {
                trits[i] = 0;
            } else if ( low == 0 ) {
                trits[i] = 1;
