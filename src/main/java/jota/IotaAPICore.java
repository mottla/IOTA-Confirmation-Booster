 ...
 * modified by theman
 */
public abstract class IotaAPICore {
    ...
    //all manual changes of this class: 
    //1.disable builder pattern and replace with custom builder
    //2.added myattachtotangle 
    //3.extended getTransactionsToApprove method
    public GetAttachToTangleResponse myattachToTangle(String trunkTransaction, String branchTransaction, Integer minWeightMagnitude, String... trytes) throws InvalidTrytesException {
        ...
        for (int i = 0; i < trytes.length; i++) {
            ...
            Transaction test = new Transaction(resultTrytes[i]);
            previousTransaction = test.getHash();
            System.out.println("----\\\\\\----");
            System.out.println("Hash " + test.getHash());
            System.out.println("Add " + test.getAddress());
            System.out.println("Val " + test.getValue());
            System.out.println("Time " + test.getTimestamp());
            System.out.println("----\\\\\\----");
        }
    ...
    //modified method
    public GetTransactionsToApproveResponse getTransactionsToApprove(Integer depth, String reference, int numWalks) {
        final Call<GetTransactionsToApproveResponse> res = service.getTransactionsToApprove(IotaGetTransactionsToApproveRequest.createIotaGetTransactionsToApproveRequest(depth, reference, numWalks));
        return wrapCheckedException(res).body();
    }

    //pass original to modified
    public GetTransactionsToApproveResponse getTransactionsToApprove(Integer depth) {
        return getTransactionsToApprove(depth, null, 0);
    }
    //original method
//    public GetTransactionsToApproveResponse getTransactionsToApprove(Integer depth) {
